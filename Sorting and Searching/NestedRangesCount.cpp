...
struct node {
	...
	void update(long value) {
		if (value < left || value > right) {
	        return; //when the given value is outside the range of the current subtree
	    }
	    if (left == right) {
	        freq++;
	    } else {
	...
	int sum_of_freq(long left, long right) {
...
int main() {
	...
	for (long i = 0; i < n; i++) {
		...
		// map the right bound to the index of the node
