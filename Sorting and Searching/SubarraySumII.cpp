...
int main() {
    ...
    Finally, we increment the count of the current prefix sum. */
    for (long i = 0; i < n; i++) {
        cin >> a;
        // update the current prefix sum
        sum += a; 
        // add the count of the prefix sum that is x less than the current prefix sum or 0 if it is not present
